...
#define CMD_SET_CONTROLS_AND_ODOM 0x05
...
typedef enum MatlabCmd {NoneCmd, OpenCmd, CloseCmd, SetVelCmd, SetOdomCmd, ReadDataCmd, SetFreeWheelsCmd};
...
struct CmdFrame 
{
  ...
  uint16_t mode;
  int16_t vel;    // Left wheel angular velocity
  int16_t servo;
...
struct CmdRespFrame 
{
  ...
  uint16_t robot_number;
  uint16_t mode;
  int16_t wRear;    // Left wheel angular velocity
  float x;        // X coordinate
  float y;
  float theta;    // Orientation
  int16_t uReg;
...
void setOdometry(float x, float y, float theta)
{
    ...
    prepareFrame(MODE_SET_ODOMETRY | MODE_MOTORS_ON, CMD_SET_CONTROLS_AND_ODOM);
}

void setControls(float vel, float servo)
{
   vel = saturateVelocity(vel);
   tx_frame.vel =  (int16_t) (vel * 256.0f);
   tx_frame.servo = (int16_t) (servo * 4096.0f);
   prepareFrame(MODE_MOTORS_ON, CMD_SET_CONTROLS_AND_ODOM);
}
...
void switchOffMotors()
{
   tx_frame.vel = 0;
   tx_frame.servo = 0;
   prepareFrame(MODE_MOTORS_OFF, CMD_SET_CONTROLS_AND_ODOM);
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
   ...
   if((ErrorParsec==0)&&(cmdNum != NoneCmd)) 
   {
      switch(cmdNum)
	  {
			...
			case SetVelCmd:
                ...
                initFrame();
				setControls((float)matrixData[0], (float)matrixData[1]);
			...
			case ReadDataCmd:
			{
				...
				if (readFrame(com.buffer_in))
				{
					...
					wRear = rx_frame->wRear / 256.0;
					x = rx_frame->x;
					y = rx_frame->y;
					th = rx_frame->theta;
                    uReg = rx_frame->uReg / 4096.0;
                    result = 1.0;
				}
   ...
   else 
   {
	    ...
	    printf("                                                      \n");
        printf("--------------        MTracker CAR MATLAB driver v.0.1      -----------------\n");
        printf("        (c) IAR 2021               \n");
		printf("                                                      \n");
        //help();
   }
