...
void CRC16Calc( unsigned char DataByte )
{
    ...
    Crc16Value = (unsigned int)(Crc16Value << 8) ^ (unsigned int)(crc_lookup[ (unsigned int) ( ((Crc16Value >> 8) & 0xFF) ^ DataByte) ]);
}
*/
...
{
    // Calculating the CRC16 by using a LookUp table
	Crc16Value = (Crc16Value << 8) ^ (crc_lookup[(((Crc16Value >> 8) & 0x00FF) ^ DataByte)]);
	//Crc16Value = (Crc16Value << 8) ^ crc_lookup[(((Crc16Value >> 8) & 0x00FF) ^ DataByte) & 0x00FF];
	return Crc16Value;
}
